...
"""Gabu 2.6 — TalesRunner HK official announcements.

Only the first valid event image is uploaded to Discord.
Keeps existing v4 announcement state and workflow compatibility.
"""
...
# Patch and system announcements
...
def split_announcements(soup, category):
    ...
    for block in blocks:
        date_index = next(
            (
                i for i, line in enumerate(block[:3])
...
def discover_event_images(item):
    ...
    for img in content.find_all("img"):
        ...
        for source in sources:
            # Relative paths use the event page URL.
            url = absolute_url(
                source,
                base_url=page_url,
            )
...
def download_event_images(item):
    """Download only the first valid image."""
    files = []
    ...
    try:
        ...
        print(
            "WARNING: Event page unavailable:",
            type(exc).__name__,
        )
        return files
    ...
    for url in candidates:
        ...
        try:
            with SESSION.get(
                ...
                if (
                    length
                    and int(length) > MAX_IMAGE_BYTES
                ):
                    print(
            ...
            if not kind or len(data) < 1024:
                print(
                    "Skipping unsupported or tiny image"
                )
                continue

            ext, mime = kind
            filename = (
                f"event_{len(files) + 1}.{ext}"
            )
...
def notify(item, changed=False):
    ...
    if item["category"] == "event":
        ...
        if images:
            # Only one attachment and one image embed.
            embeds[0]["image"] = {
                "url": "attachment://" + images[0][0]
            }
        else:
            print(
                "No suitable event image; "
                "sending text and official link only"
            )
...
def main():
    ...
    # Existing v4 state remains compatible.
    ...
    for category, items in all_items.items():
        ...
        print(
            f"{category}: sent "
            f"{min(8, len(pending))} notifications"
        )

        if len(pending) > 8:
            print(
                f"{category}: "
                f"{len(pending) - 8} changes remaining"
            )

    print(
        "\nGabu 2.6 completed successfully"
    )
